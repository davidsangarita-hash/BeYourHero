/* ============================================================
   ...
   Clean logo rendering — no onerror HTML artifacts
   ============================================================ */
...
const Logos = (() => {
  ...
  const LEAGUE_SVG = {
    ...
    eredivisie:       `<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><circle cx="50" cy="50" r="48" fill="#FF6600" stroke="#003087" stroke-width="3"/><text x="50" y="44" text-anchor="middle" fill="#fff" font-size="9" font-weight="bold" font-family="Arial">EREDIVISIE</text><line x1="20" y1="52" x2="80" y2="52" stroke="#003087" stroke-width="2"/></svg>`,
    ...
    championship:     `<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><rect x="2" y="2" width="96" height="96" rx="10" fill="#2c1654" stroke="#a0e6ff" stroke-width="2"/><text x="50" y="38" text-anchor="middle" fill="#a0e6ff" font-size="7" font-weight="bold" font-family="Arial">CHAMPIONSHIP</text><circle cx="50" cy="62" r="14" fill="none" stroke="#a0e6ff" stroke-width="2"/><circle cx="50" cy="62" r="6" fill="#a0e6ff"/></svg>`,
    segunda_division: `<svg viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg"><rect x="2" y="2" width="96" height="96" rx="10" fill="#c0392b" stroke="#FFD700" stroke-width="2"/><text x="50" y="50" text-anchor="middle" fill="#FFD700" font-size="28" font-weight="bold" font-family="Arial">2ª</text><text x="50" y="72" text-anchor="middle" fill="#fff" font-size="9" font-family="Arial">SEGUNDA DIV.</text></svg>`,
  ...
  function generateSVGShield(club) {
    ...
    const uid    = (club.id||'x').replace(/[^a-z0-9]/gi,'');
    return `<svg viewBox="0 0 100 120" xmlns="http://www.w3.org/2000/svg">
      <defs><linearGradient id="sg${uid}" x1="0%" y1="0%" x2="100%" y2="100%">
        <stop offset="0%" stop-color="${color1}"/><stop offset="100%" stop-color="${color2}"/>
      </linearGradient></defs>
      <path d="M50 5L95 20L95 70Q95 105 50 118Q5 105 5 70L5 20Z" fill="url(#sg${uid})" stroke="rgba(255,255,255,0.22)" stroke-width="2"/>
      <path d="M50 14L86 27L86 68Q86 98 50 108Q14 98 14 68L14 27Z" fill="none" stroke="rgba(255,255,255,0.1)" stroke-width="1"/>
      <text x="50" y="72" text-anchor="middle" fill="#fff" font-size="${short.length>3?12:17}" font-weight="bold" font-family="Arial">${short}</text>
    </svg>`;
  }
  ...
  /**
   * Returns a self-contained div: SVG underneath, real img on top.
   * If img loads → real logo shows. If it fails → img removes itself, SVG visible.
   * NO onerror HTML injection — avoids parser artifacts.
   */
  function getLeagueLogo(leagueId) {
    const url = LEAGUE_URLS[leagueId];
    const svg = LEAGUE_SVG[leagueId] || LEAGUE_SVG['premier_league'];
    if (!url) return `<span class="logo-svg-only">${svg}</span>`;
    return `<span class="logo-real-wrap">
      <span class="logo-svg-layer">${svg}</span>
      <img class="logo-real-img" src="${url}" alt="" loading="lazy" onerror="this.remove()">
    </span>`;
  }

  function getClubShield(club) {
    if (!club) return generateSVGShield({ id:'unk', shortName:'?', color:'#1a1a2e' });
    if (club.apiId) {
      const url = `https://media.api-sports.io/football/teams/${club.apiId}.png`;
      return `<span class="logo-real-wrap">
        <span class="logo-svg-layer">${generateSVGShield(club)}</span>
        <img class="logo-real-img" src="${url}" alt="${club.shortName||''}" loading="lazy" onerror="this.remove()">
      </span>`;
    }
  ...
  function getFlagSVG(code) {
    return FLAG_SVG[code] || `<svg viewBox="0 0 60 40" xmlns="http://www.w3.org/2000/svg"><rect width="60" height="40" fill="#1a1a2e" rx="2"/><text x="30" y="26" text-anchor="middle" fill="#666" font-size="12">?</text></svg>`;
  }
